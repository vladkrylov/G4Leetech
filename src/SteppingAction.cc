...
//		if(aStep->GetTrack()->GetDefinition()->GetPDGEncoding() == 11) {
			...
			analysisManager->FillNtupleDColumn(BranchId=13,aStep->GetTrack()->GetDynamicParticle()->GetCharge());
	...
	G4LogicalVolume* volumelog
	    = aStep->GetPreStepPoint()->GetTouchableHandle()
	      ->GetVolume()->GetLogicalVolume();
	if (volumelog->GetName() != "DDLV") return;
	if (volumelog->GetName() == "DDLV"){
